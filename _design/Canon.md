...
**Noticing this is not a GM responsibility.** It's the same automated pass described in [[Glossary]] under Signal: when session notes are taken in, every new fact is checked against the record for possible relationships, possible investment shifts, and — the same mechanism, a third question — possible collision with an existing canon fact. If intake surfaces a table fact that looks like it changes a documented outcome, a Ticket asks the GM to confirm it's deliberate. The GM decides; the system watches.
...
**Supersession is the one genuinely new piece, and detecting it is the system's job, not the GM's.** During session-note intake, every new fact already gets checked for possible relationships and possible investment shifts — this is the same Signal pass, with a third question added: does this fact collide with an existing canon fact. When a table fact changes a *documented* canon outcome, the canon fact is never edited or removed — it stays exactly as the source recorded it, permanently. The new table fact carries a `supersedes` relationship to it instead, and intake raises a **Ticket** so the GM confirms the collision was deliberate rather than being expected to have caught it unassisted. Everything downstream that assumed the original outcome is then reachable by following that relationship, rather than needing its own decay-tracking field. The relationship is GM-side bookkeeping only — it has no in-fiction referent, so there's nothing about it that would ever be player-visible; that's not a gap to close, it's outside what the model needs to represent at all.
